...
def run_sga_simulation(config, run_id=0):
    ...
    for k in range(1, iterations + 1):
        # 使用为SGA设计的、正确的违背计算函数
        history.append(calculate_sga_violation(initial_state, params, photon_num))
        ...
        val_plus = calculate_sga_violation(initial_state, params_plus, photon_num)
        val_minus = calculate_sga_violation(initial_state, params_minus, photon_num)
...
def run_sga_chsh_simulation(config, run_id=0):
    ...
    for k in range(1, iterations + 1):
        # 使用为SGA设计的、正确的违背计算函数
        history.append(calculate_sga_chsh_violation(initial_state, params, photon_num))
        ...
        val_plus = calculate_sga_chsh_violation(initial_state, params_plus, photon_num)
        val_minus = calculate_sga_chsh_violation(initial_state, params_minus, photon_num)
