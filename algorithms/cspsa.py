...
from algorithms.simulation_utils import calculate_cspsa_violation
from algorithms.simulation_utils import calculate_cspsa_chsh_violation
...
def run_cspsa_simulation(config, run_id=0):
    ...
    for k in range(1, iterations + 1):
        # 修正：调用正确的函数
        history.append(calculate_cspsa_violation(initial_state, params))
        ...
        # 修正：调用正确的函数
        val_plus = calculate_cspsa_violation(initial_state, params_plus)
        val_minus = calculate_cspsa_violation(initial_state, params_minus)
...
def run_cspsa_chsh_simulation(config, run_id=0):
    ...
    for k in range(1, iterations + 1):
        # 修正：调用正确的函数
        history.append(calculate_cspsa_chsh_violation(initial_state, params))
        ...
        # 修正：调用正确的函数
        val_plus = calculate_cspsa_chsh_violation(initial_state, params_plus)
        val_minus = calculate_cspsa_chsh_violation(initial_state, params_minus)
