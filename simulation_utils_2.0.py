...
# 这部分是正确的，保持不变
# =============================================================================
photonn_set = 100000000  # 设置光子数的基准值
...
def calculate_cspsa_violation(rho, params):
...
def calculate_cspsa_uncer_violation(rhok, params, standeviation):
    """为CSPSA计算违背值 (基于'bell'算符)。"""
    rho=rhok
    ket0, ket1 = qt.basis(2, 0), qt.basis(2, 1)
    theta_k = np.random.normal(loc=np.pi/4, scale=standeviation)
    psi_pure = np.cos(theta_k) * qt.tensor(ket0, ket0) + np.sin(theta_k) * qt.tensor(ket1, ket1)
    rho = qt.ket2dm(psi_pure)
...
def calculate_cspsa_mis_violation(rho, params):
    """为CSPSA计算违背值 (基于'bell'算符)。"""
    param_pairs = np.reshape(params, (5, 2))
    P_A1, P_A1_perp = create_cspsa_projectors(param_pairs[0][0], param_pairs[0][1])
    P_A2, P_A2_perp = create_cspsa_projectors(param_pairs[1][0], param_pairs[1][1])
...
def calculate_cspsa_chsh_violation(rho, params):
    """为CSPSA计算违背值 (基于'bell'算符)。"""
...
def calculate_sga_violation(rho, params):
    """
    为SGA计算违背值。
...
def calculate_sga_chsh_violation(rho, params):
    """
    为SGA计算违背值。
    参数是10个实数，每对(theta, phi)通过一个特殊的映射定义一个测量。
    """
